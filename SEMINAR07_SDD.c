...
void filtreazaHeap2(Heap heap, int pozitieNod) {
	int pozStg=2*pozitieNod+1;
	int pozDr=2*pozitieNod+2;
	int pozMax=pozitieNod;

	if (pozStg<heap.nrMasini && heap.masini[pozStg].id>heap.masini[pozitieNod].id) {
		pozMax=pozStg;
	}

	if (pozDr<heap.nrMasini && heap heap.masini[pozDr].id>heap.masini[pozitieNod].id) {
		pozMax=pozDr;
	}
	if (pozMax!=pozitieNod) {
		Masina aux=heap.masini[pozitieNod];
		heap.masini[pozitieNod]=heap.masini[pozMax];
...
int main() {
	...
	Heap heap = citireHeapDeMasiniDinFisier("/Users/mateiavram/Desktop/facultate/Structuri de Date/Seminar8_SDD/masini_arbore.txt");
	afisareHeap(heap);
	extrageMasina(&heap);
	extrageMasina(&heap);
	extrageMasina(&heap);
